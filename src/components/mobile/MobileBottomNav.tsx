...
import { Home, Disc3, Library, Ticket, User, LayoutDashboard, Users, Wallet, Handshake, Activity, Banknote } from "lucide-react";
...
const PARTNER_NAV = [
  { href: "/m/partner", label: "Partner", icon: Handshake },
  { href: "/m/partner/activity", label: "Activity", icon: Activity },
  { href: "/m/partner/payout", label: "Payout", icon: Banknote },
  { href: "/m/profile", label: "Profile", icon: User },
];
...
export default function MobileBottomNav() {
  ...
  const items = role === "musician" ? MUSICIAN_NAV : role === "partner" ? PARTNER_NAV : MEMBER_NAV;
  ...
  return (
    ...
    >
      {items.map(({ href, label, icon: Icon }) => {
        const active = href === "/m" || href === "/m/dash" || href === "/m/partner" ? pathname === href : pathname.startsWith(href);
        return (
          <Link
            key={href}
            href={href}
